...
// wish cnt
$wish_sql = "select count(*) as cnt from shop_wish where gs_id='$index_no' and mb_id = '".$member["id"]."'";
$wish_cnt = sql_fetch($wish_sql);
...
if($is_soldout) {
	...
	}else if($tc["counseling_type"] == 2){
		$script_msg = "상담중으로 상담이 불가능합니다.";
	}else if($tc["counseling_type"] == 3){
		$script_msg = "상담종료로 상담이 불가능합니다.";
	}else{
		$script_msg = "";
	}
