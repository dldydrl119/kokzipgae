...
<div class="bd list1 marb10">
	<fieldset style="padding:0 10px;">
		...
		<tr height="40">
			...
			<strong><span class="fc_255">"<?php echo $ss_tx; ?>"</span> 综合搜索结果总<span class="fc_255"><?php echo number_format($total_count); ?></span>搜索了狗的商品。 <strong></td>
		</tr>
		<tr height="40">
			<td class="tal padr10 padl10">
				<span class="padr5">搜索</span>
...
<div class="tab_sort">
	<ul>
		<?php echo $sort_str; // Tabmene ?>
	</ul>
	<select id="page_rows" onchange="location='<?php echo "{$_SERVER['SCRIPT_NAME']}?{$qstr3}";?>&page_rows='+this.value;">
		<?php echo option_selected(($mod*5),  $page_rows, /'5 排序行'); ?>
		<?php echo option_selected(($mod*10), $page_rows, /'10 排序行'); ?>
		<?php echo option_selected(($mod*15), $page_rows, /'15 排序行'); ?>
	</select>
</div>
...
<?php if(!$total_count) { ?>
<div class="empty_list bb">没有资料。</div>
<?php } ?>
