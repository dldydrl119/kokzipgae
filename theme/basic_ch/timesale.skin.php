...
function CountDownTimer(dt, id)
{
	...
	function showRemaining() {
		...
		var str = "";
		str += '剩下的时间 <span class="num">'+days + '</span> 一天 ';
		str += '<span class="num">'+pad(hours,2) + '</span> 时间 ';
		str += '<span class="num">'+pad(minutes,2) + '</span> 分 ';
		str += '<span class="num">'+pad(seconds,2) + '</span> 蜡烛';
...
<div class="tab_sort">
	<span class="total">全部商品 <b class="fc_90" id="total"><?php echo number_format($total_count); ?></b>개</span>
	<ul>
		<?php echo $sort_str; // 标签菜单 ?>
	</ul>
	<select id="page_rows" onchange="location='<?php echo "{$_SERVER['SCRIPT_NAME']}?{$qstr3}";?>&page_rows='+this.value;">
		<?php echo option_selected(($mod*5),  $page_rows, /'5排序行'); ?>
		<?php echo option_selected(($mod*10), $page_rows, /'10排序行'); ?>
		<?php echo option_selected(($mod*15), $page_rows, /'15排序行'); ?>
	</select>
</div>
