...
Choose</option>
		...
		if(is_admin()) {
			...
			$option .= '<input type="checkbox" name="btype" value="1" id="ids_btype" class="css-checkbox lrg"'.$checked.'><label for="ids_btype" class="css-label padr10">Notice</label>';

			$checked = '';
			if($write['issecret']=='Y') { $checked = ' checked'; }
			$option .= '<input type="checkbox" name="issecret" value="Y" id="ids_issecret" class="css-checkbox lrg"'.$checked.'><label for="ids_issecret" class="css-label padr10">Secret writing</label>';
		} else {
...
Contents</th>
	...
	<div class="btn_confirm">
		<input type="submit" value="Confirm" class="btn_medium">
		<?php if($w == 'u' || $w == 'r') { ?>
