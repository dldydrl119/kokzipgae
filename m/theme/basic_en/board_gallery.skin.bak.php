<?php
if(!defined("_TUBEWEB_")) exit; // Individual pages can not be accessed.
?>
...
<div class="m_gall">
	...
	<select name="faq_type" class="faq_sch" onchange="location=this.value;">
		<option value="<?php echo TB_MBBS_URL; ?>/board_list.php?boardid=<?php echo $boardid; ?>">Full view</option>
		<?php
	...
	<ul id="m_gall_ul">
		...
		for($i=0; $row=sql_fetch_array($rst); $i++) {
			...
			$bo_subject = '<strong class="fc_eb7">[Notice]</strong> '.get_text($row['subject']);
	...
	<div class="btn_confirm">
		<a href="<?php echo TB_MBBS_URL; ?>/board_write.php?boardid=<?php echo $boardid;?>" class="btn_medium">Writing</a>
	</div>
	<?php } ?>
...
/*
setTimeout Check value
*/
var ing = {'lazyloadCallback' : false};
/*
masonry Option value
*/
var masonryOptions = {itemSelector : ".item", columnWidth : ".item", percentPosition : true};
$(function(){
	/*
	Lazy Load Apply Plug-in
	(load Specifying callback functions through methods / The rearrangement of the masonry is not a layout method
	destroy method-based after removing masonry Reproduction of masonry.
	If you reapply after destroy, there is a problem with scrolling to the top of the PC browser
	navigator.If userAgent is not mobile by checking its value
	I saved the height of the scroll Move the machine to the stored height value of the scroll after the mask is completed.)
	*/
		...
		load : function(){
			...
			if($masgall){
				...
				ing['lazyloadCallback'] = window.setTimeout(function(){
					...
					console.log('function duplicate check log - rearrangement of masonry!');
				}, 100);
			}
		}
	});
	/*
	masonry First application
	*/
