...
<div class="sp_wrap">
	...
	<div class="sp_sns">
		满意度 : <?php echo $aver_score; ?>% <span class="hline"></span>评论 : <?php echo number_format($item_use_count); ?>案件
	</div>
	...
	<div class="sp_tbox">
		<ul>
			<li class='tlst strong'>订货期权</li>
			<li class='trst fs11 padt2'>以下选项是必须选择的选项。</li>
		</ul>
	</div>
	...
	<div class="sp_tbox">
		<ul>
			<li class='tlst strong'>追加构成</li>
			<li class='trst fs11 padt2'>想要追加购买的话请选择。</li>
		</ul>
	</div>
			...
			<li class="sit_opt_list">
				...
				<input type="hidden" class="io_stock" value="<?php echo $gs['stock_qty']; ?>">
					<ul>
						...
						<li class='trst'>
							<dl>
								<dt class='fl padr3'><button type="button" class="btn_small grey">减少</button></dt>
								<dt class='fl padr3'><input type="text" name="ct_qty[<?php echo $gs_id; ?>][]"
								value="<?php echo $odr_min; ?>" title="수량설정"></dt>
								<dt class='fl padr3'><button type="button" class="btn_small grey">增加</button><dt>
								<dt class='fl padt4 tx_small'> (剩余数量 : <?php echo $gs['stock_mod'] ? $gs['stock_qty']./'个' : /'无限制'; ?>)</dt>
							</dl>
						</li>
					</ul>
				</div>
	...
	<div id="sit_tot_views" class="dn">
		<div class="sp_tot">
			<ul>
				<li class='tlst strong'>枪 合计金额</li>
				<li class='trst'><span id="sit_tot_price" class="trss-amt"></span><span class="trss-amt">韩元</span></li>
			</ul>
		</div>
	</div>
	<?php } ?>
	...
	<div class="sp_tab">
		<nav role="navigation">
			<ul>
				<li id='d1' class="active"> <a href="javascript:chk_tab(1);">产品信息</a> </li>
				<li id='d2'> <a href="javascript:chk_tab(2);">购买后期</a> </li>
				<li id='d3'> <a href="javascript:chk_tab(3);">Q&A</a> </li>
				<li id='d4'> <a href="javascript:chk_tab(4);">退货/交换</a> </li>
			</ul>
		</nav>
	</div>

	<div class="sp_msgt">以下商品信息可能与期权及赠品信息等实际商品存在差异</div>
	<div id="v1">
		<div class="sp_vbox">
			<ul>
				<li class='tlst'>&#183;&nbsp;&nbsp;商品编号</li>
				<li class='trst'><?php echo $gs['gcode']; ?></li>
			</ul>
			<ul>
				<li class='tlst padt2'>&#183;&nbsp;&nbsp;制造商</li>
				<li class='trst padt2'><?php echo $gs['maker']; ?></li>
			</ul>
			<ul>
				<li class='tlst padt2'>&#183;&nbsp;&nbsp;原产地 (生产国)</li>
				<li class='trst padt2'><?php echo $gs['origin']; ?></li>
			</ul>
			<ul>
				<li class='tlst padt2'>&#183;&nbsp;&nbsp;A/S 可否</li>
				<li class='trst padt2'><?php echo $gs['repair']; ?></li>
			</ul>
		</div>
		<div class="sp_vbox_mr">
			<ul>
				<li class='tlst'>电子商务商品信息发布</li>
				<li class='trst'><a href="javascript:chk_show('extra');" id="extra">看 <span class='im im_arr'></span></a></li>
			</ul>
		</div>
	...
	<div id="v2" style="display:none;">
		<?php echo mobile_goods_review("购买后期", $item_use_count, $gs_id); ?>
	</div>
...
function fsubmit_check(f)
{
    ...
    if(min_qty > 0 && sum_qty < min_qty) {
		alert("订货期权个数总和 "+number_format(String(min_qty))+"请订购狗以上。");
        return false;
    }

    if(max_qty > 0 && sum_qty > max_qty) {
		alert("订货期权个数总和 "+number_format(String(max_qty))+"请订购狗以下。");
        return false;
    }
...
function chk_show(name) {
	...
	if(old!=submenu) {
		...
		submenu.display='';
		eval("extra").innerHTML = "关闭";
		old = submenu;

	} else {
		submenu.display='none';
		eval("extra").innerHTML = "看";
		old = '';
	}
}
