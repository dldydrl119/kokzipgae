...
<div class="m_gall">
	...
	<select name="faq_type" class="faq_sch" onchange="location=this.value;">
		<option value="<?php echo TB_MBBS_URL; ?>/board_list.php?boardid=<?php echo $boardid; ?>">全面查看</option>
		<?php
	...
	<?php
	if(!$total_count) {
		echo "<p class=\"empty_list\">没有留言。</p>";
	} else {
	?>
	<ul id="m_gall_ul">
		<?php
		...
		for($i=0; $row=sql_fetch_array($rst); $i++) {
			...
			$bo_subject = '<strong class="fc_eb7">[空地]</strong> '.get_text($row['subject']);
	...
	<div class="btn_confirm">
		<a href="<?php echo TB_MBBS_URL; ?>/board_write.php?boardid=<?php echo $boardid;?>" class="btn_medium">写作</a>
	</div>
	<?php } ?>
	...
	<div class="bottom_sch">
		...
		<input type="submit" value="搜索" class="btn_small grey">
	</div>
	</form>
</div>
...
$(function(){
	/*
	采用Lazy Load插头
	(load 通过Messord指定呼叫包 / masonry的财政热是 masonry的财政热是
	利用desroy方法 去除masonry后用于masonry的再适用处理.
	destroy后重新使用时,电脑浏览器存在滚动至顶端的问题
	核对navigator.userAgent的价格,不是手机而是手机
	储存scroll的值,masonry应用完毕后移动至存储的scroll值。.)
	*/
		...
		load : function(){
			...
			if($masgall){
				...
				ing['lazyloadCallback'] = window.setTimeout(function(){
					...
					console.log('函数重复格子登录 - masonry 重新排列!');
				}, 100);
			}
		}
	});
