 ...
 */
	...
	if($success)
	{
		...
		if($xml->result->resultcode == '00') {
			...
			$mb_sns_id		=	$xml->response->id; 
			$mb_id				=	$xml->response->email;
			$mb_email			=	$xml->response->email;
			$mb_name			=	$xml->response->name;
			$mb_nick			=	$xml->response->nickname;
			$mb_img			=	$xml->response->profile_image;
			$mb_img_type	=	"S";
			$mb_birth_month	=	substr($xml->response->birthday,0,2);
			$mb_birth_day	=	substr($xml->response->birthday,3,2);
			$mb_phone		=	replace_tel($xml->response->mobile);	//핸드폰

			$mb_gender		=	$xml->response->gender;
