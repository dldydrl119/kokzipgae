...
function get_sns_share_link($sns, $url, $title, $image_url)
{
    ...
    switch($sns) {
        ...
        case 'twitter':
			...
			$str = '<a href="'.$twitter_url.'" onclick="'.$str.'" target="_blank"><img src="'.$image_url.'"><p>트위터</p></a>'.PHP_EOL;
			break;
        case 'naver':
            ...
            break;
        case 'kakao':
			$str	= '<script src="https://developers.kakao.com/sdk/js/kakao.js"></script>'.PHP_EOL;
			$str .= ' <a id="kakao-link-btn" href="javascript:sendLink();"><img src="'.$image_url.'" alt="kakao"><p>카카오톡</p></a>'.PHP_EOL;
			$str .= ' <script type="text/javascript">'.PHP_EOL;
			$str .= '		Kakao.init(\'ce5d5d011fb8a619d3117723274ea337\');'.PHP_EOL;
			$str .= '		Kakao.isInitialized();'.PHP_EOL;
			$str .= '		function sendLink() {'.PHP_EOL;
			$str .= '			Kakao.Link.sendScrap({'.PHP_EOL;
			$str .= '				requestUrl: \''.str_replace(":443","",$url).'\','.PHP_EOL;
			$str .= '			})'.PHP_EOL;
			$str .= '		}'.PHP_EOL;
			$str .= '</script>'.PHP_EOL;
            break;
		...
		case 'naverline':
			...
			$str = '<a href="'.$naverline_url.'" onclick="'.$str.'" ><img src="'.$image_url.'"><p>라인</p></a>'.PHP_EOL;
            break;
		case 'naverband':
...
function change_order_status_4($od_no, $delivery='', $delivery_no='')
{
	...
	$sql = " update shop_teacher
				set counseling_type = '2'
				where index_no = '".$gs["tc_code"]."'";
	sql_query($sql);
...
function change_order_status_5($od_no)
{
	...
	// 알림 등록
	$memo	=	"'".$gs["gname"]."- ".$tc["name"]."' 상담을 완료 하였습니다..";
	boardRegData('43',$od["mb_id"], addslashes($memo));


	//선생님 상태 변경
	$sql = " update shop_teacher
				set counseling_type = '1'
				where index_no = '".$gs["tc_code"]."'";
	sql_query($sql);
