...
function get_buy_button_unit($msg, $gs_id, $sw_direct){
	...
	if($msg) {
		$str .= "<a href=\"javascript:alert('$msg');\"  class=\"".$sw_css."\" id=\"".$sw_iddd."\" >".$sw_img."</a></span>";
	} else {
		if($sw_direct == "wish") {
			$str .= "<a href=\"javascript:item_wish(document.fbuyform);\" class=\"".$sw_css."\"  id=\"".$sw_iddd."\">".$sw_img."</a>";
		} else {
			$str .= "<a href=\"javascript:fbuyform_submit('".$sw_direct."');\" class=\"".$sw_css."\"  id=\"".$sw_iddd."\">".$sw_img."</a>";
		}
	}
...
function counseling_sticker($counseling_type,$device){
	...
	if($device == "pc"){
		...
		if($counseling_type == 1){
			...
			$str = "<div class=\"state state3\">상담종료<img src=\"".TB_IMG_URL."/content_tel.png\" alt=\"tel\"></div>";		
		}else{
			$str = "<div class=\"state state1\">상담가능<img src=\"".TB_IMG_URL."/content_tel.png\" alt=\"tel\"></div>";		
		}
	}
	...
	if($device == "mo"){
		...
		if($counseling_type == 1){
			$str = "<div class=\"state state1\"><span>상담가능</span><img src=\"/img/btn/content_tel.png\" ></div>";
		}else if($counseling_type == 2){
			$str = "<div class=\"state state2\"><span>상담중</span><img src=\"/img/btn/content_tel.png\" ></div>";		
		}else if($counseling_type == 3){
			$str = "<div class=\"state state3\"><span>상담종료</span><img src=\"/img/btn/content_tel.png\" ></div>";		
		}else{
			$str = "<div class=\"state state1\"><span>상담가능</span><img src=\"/img/btn/content_tel.png\" ></div>";
		}
	}
