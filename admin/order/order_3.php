...
<div class="tbl_head01">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$sql = " select * {$sql_common} {$sql_search} and od_id = '{$row['od_id']}' order by a.index_no ";
		$res = sql_query($sql);
		$rowspan = sql_num_rows($res);
		for($k=0; $row2=sql_fetch_array($res); $k++) {
			$gs = unserialize($row2['od_goods']);
			$tc	=	sql_fetch("select a.*, b.mem_img, b.cellphone, b.nickname from shop_teacher a, shop_member b where a.mb_code = b.index_no and a.index_no = '".$gs["tc_code"]."'");
	?>
...
<div class="tbl_frm01">
	...
	<tr>
		<td>
			<input type="submit" name="act_button" value="상담중" class="btn_medium red" onclick="document.pressed=this.value">
		</td>
	</tr>
	</tbody>
	</table>
...
<div class="information">
	...
	<div class="content">
		...
		<div class="desc01 accent">
			<p>ㆍ주문리스트 내 선택된 주문의 상태를 <em>"입금완료 &gt; 상담준비 &gt; 상담중 &gt; 상담완료 &gt; 구매확정"</em> 순으로 변경됩니다.</p>
			<p>ㆍ상담준비 상태의 주문은 "상담중" 상태로만 변경할 수 있으며, 주문의 일부 상품만 부분적으로 "상담중" 상태로 변경 가능합니다.</p>
		</div>
		<div class="hd">ㆍ출고전 취소할 수 있나요?</div>
		<div class="desc01 accent">
			<p>ㆍ주문리스트에서 상담준비 상태의 주문을 취소/환불 처리가 불가능합니다.</p>
